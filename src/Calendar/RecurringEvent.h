...
public:
     ...
     */
    RecurringEvent(std::string title_, time_t startDateUtc_, time_t duration_, time_t timeBetweenEvents_, time_t repeatTill_);
    /**
     * @throws invalid_argument if duration_ is less than 1 or if timeBetweenEvents_ is less than 1
     */
    RecurringEvent(std::string title_, time_t startDateUtc_, time_t duration_, time_t timeBetweenEvents_);
     ...
     */
    RecurringEvent(std::istringstream &input);
    ...
    std::shared_ptr<SingleEvent> eventExists(time_t start, time_t end, time_t repeat, time_t repeatTill_) override;

    /**
     * Removes event from sequence of events generated by this
     * @param event event to free from sequence
     * @param actionType how many events to free
     * @return std::shared_ptr to freed event
     */
    std::shared_ptr<Event> freeRecurringItemEvent(const std::shared_ptr<RecurringItemEvent> &event, actionType actionType);

    virtual std::shared_ptr<Event> freeSelf(actionType actionType);
    ...
    time_t findFreeSpace(const EventSet<std::shared_ptr<Event>> &events) override;

private:
    std::shared_ptr<RecurringEvent> state;

    std::shared_ptr<RecurringEvent> getFirstNode();

    /**
     * Gets RecurringEvent node that generated the event
     * @param recurringItem RecurringEvent to find parent of
     * @return std::shared_ptr to node of this Recurring Event that generated recurringItem
     ...
     */
    std::shared_ptr<RecurringEvent> getParentOfItem(const std::shared_ptr<RecurringItemEvent> &recurringItem);
     ...
     * @return Freed event
     */
    std::shared_ptr<Event> freeOnlyOneRecurringItemEvent(const std::shared_ptr<RecurringItemEvent> &event);
     ...
     */
    std::shared_ptr<RecurringEvent> freeThisAndNextRecurringItemEvent(const std::shared_ptr<RecurringItemEvent> &event);
     ...
     */
    std::shared_ptr<RecurringItemEvent> getSingle(time_t start);
    ...
    std::weak_ptr<RecurringEvent> parentNode;

    //Child node - always has event start after this repeatTill
    std::shared_ptr<RecurringEvent> childNode = nullptr;
//GETTERS AND SETTERS
public:
